...
// =======================================================================
// ALERTA SUPREMA DE GAMBIARRA
// =======================================================================


router.get('/therapist-weekly-history/:idtrait/:iduser', verifyToken, async (req, res) => {
  try {
    const { idtrait, iduser } = req.params;
    ...
    const history = await db('tracking')
      ...
      .where({ 
        idtraits: idtrait,
        idpaciente_criador: iduser // Filtra apenas pelos dados deste paciente
      })
      .andWhere('dia_de_registro', '>=', dateString)
      .orderBy('dia_de_registro', 'asc');
...
// Retorna: Array [{ intensidade: 1, frequencia: 5 }, { intensidade: 2, frequencia: 10 }...]
// =======================================================================
router.get('/therapist-frequency/:idtrait/:userid', verifyToken, async (req, res) => {
  try {
    const { idtrait, userid } = req.params;
...
// Retorna: { totalTraits: 5, completedToday: 2, percentage: 40 }
// =======================================================================
router.get('/therapist-daily-completion/:userid', verifyToken, async (req, res) => {
  try {
...
// Retorna: { average: 3.5 }
// =======================================================================
router.get('/therapist-weekly-average/:userid', verifyToken, async (req, res) => {
  try {
    const { userid } = req.params;
    ...
    const result = await db('tracking')
      .avg('intensidade as media')
      .where({ idpaciente_criador: userid })
      .andWhere('dia_de_registro', '>=', dateString)
      .first();
